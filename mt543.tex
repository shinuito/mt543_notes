...
As $A$ and (matrix 3) both $\in \gun$, so is left hand matrix above.
...
This is clear since any matrix of the form (matrix 4) with $\det =1$ must be $I_n$. Analogous arguments show 
...
For a topological space $X$ we consider $\alpha:G\to \underbrace{\homeo{X}}{\text{group of "homeomorphisms"}}$.

\ul{Examples} 
\begin{itemize}
\item[i)]\glnr acts on $\rR^n$ by right or left multiplication. $\underbrace{R}{\text{right multiplication}}:\glnr \to \underbrace{\operatorname{Isom}(\rR^n)}{\text{group of linear isomorphisms of } \rR^n}$
\item[ii)] $\underbrace{\zZ_2}{=\{\pm 1\}}$ acts on $\rR$ by flips about $0$. (picture of real line arrows indicating flips)
...
\[\underbrace{\pmqty{\cos \theta & \sin \theta \\ -\sin \theta & \cos \theta}}{generic element of \gsok{2}}\pmqty{x \\ y} = \underbrace{\pmqty{\cdot \\ \cdot}}{again element of S^1}\]
...
\item A topological space is path-connected if you can join any two points by a continuous path ($p:\underset{[0,1]}{\text{standard metric topology}}\to (X,T)$)
