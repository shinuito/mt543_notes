...
\renewcommand{\thethm}{\arabic{subsection}.\arabic{subsection}.\arabic{thm}}
...
\begin{itemize}
\item $A\in \mnh$ then $R_A:\hH^n\to\hH^n$ given by $R_a(\vec{x})=\vec{x}A.$  is \hH - linear (assuming coefficients in \hH multiply on vectors from the left, c row vector). Left multiplication is not in general \hH linear.
\item Under the real linea isomorphosm $\theta_n: \cC^n \to \rR^{2n},$ $\theta _n (a_1+ib_1, \ldots , a_n+ib_n )=(a_1,b_1,\ldots, a_n,b_n)$. Any complex-linear map $\cC^n\to \cC^n$ corresponds to a real-linear map $\rR^{2n}\to \rR^{2n}$ and in terms of matrices (an right multiplication) $A\in \mnc$ corresponds to some matrix $\rho_n(A)\in \mr{2n}$.
...
\begin{cor}
(1.1.11) The image of $\rho_n:\mnc \to \mr{2n}$ is the set of all of all matrices in \mr{2n} which commute with \cin .
\end{cor}
...
\begin{prop}
(1.1.13) There is an injective \cC linear map $\psi_n:\mnh \to \mc{2n}$ s.t. the following square commutes:
...
\begin{cor}
1.1.14 There is an injective group homomorphism $\psi_n:\glnh \to \glc{2n}$.
\end{cor}
\begin{proof}
...
Composing $\rho_{2n}$ and $\psi_n$ gives
 ...
 \begin{cor}
1.1.15 There is an injective \rR linear map resp. group homomorphism given by $\rho_{2n}\circ \psi_n:\mnh \to \mr{4n}$ resp. $\rho_{2n}\circ \psi_n:\glnh \to \glr{4n}$.
\end{cor}
