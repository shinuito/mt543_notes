...
\begin{thm}
\gson acts transitively by right (or left) multiplication on $\sph{n-1}\subset \rR^n$, and given any point $x\in \sph{n-1}$ there is a continuous path $p:[0,1]\to\gson$ s.t. $p(0)=I_n$ and $R_{p(1)}(e_1)=x$ ($n\geq 2$).
\end{thm}
