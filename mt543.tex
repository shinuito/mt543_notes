...
\usepackage{physics}
\usepackage{tikz-cd}
\usepackage{mathtools} % for text over and under iff etc
...
\begin{itemize}
\item There is a canonical \cC linear isomorphism $\phi_n:\hH^n\to \cC^n$ given by $\phi_n(z_1+w_1 j, \ldots, z_n+w_n j)=(z_1,w_1,\ldots,z_n, w_n).$
...
So the claim is true. $\therefore A$ is invertible $\underset{\text{claim}}{\iff} \psi_n$ is invertible $\underset{\text{elementary linear algebra}}{\iff} \det \psi_n(A)\neq 0 \underset{\text{defn 1.1.16}}{\iff} \det A \neq 0$  
\end{proof}
...
Recall that an inner product on a vector space $V$ over \fF is a map $<,>:V\times V \to\fF$ which is bilinear (i.e. linear in each entry) and is positive definite, i.e. $\ipm{v}{v} \geq 0 \forall v\in V$. If $v\neq 0, \ipm{v}{v}>0$. e.g. dot product in $\rR^n$.
...
$\ipm{x}{y}_\cC = \ipm{\theta_n(x)}{\theta_n(y)}_\rR+i\ipm{\theta_n(x)}{\theta_n(iy)}_\rR$.
